...
//	private boolean mTestDelayElapsed; // add a timer for testing
	...
	public void onCreate() {
		...
		LinphoneCoreFactory.instance().enableLogCollection(!(Constants.DISABLE_EVERY_LOG));
		...
		Bitmap bm = null;
		try {
			bm = BitmapFactory.decodeResource(getResources(), R.drawable.icon2);
		} catch (Exception e) {
		}
		mNotif = Compatibility.createNotification(this, mNotificationTitle, "", 0, R.drawable.icon2, bm, mNotifContentIntent, true,notifcationsPriority);
		...
		LinphoneManager.getLc().addListener(mListener = new LinphoneCoreListenerBase(){
			...
			public void callState(LinphoneCore lc, LinphoneCall call, LinphoneCall.State state, String message) {
				...
				if (state == State.StreamsRunning) {
					...
					if (Constants.ENABLE_CALL_NOTIFICATION)
			...
			public void globalState(LinphoneCore lc,LinphoneCore.GlobalState state, String message) {
				if (state == GlobalState.GlobalOn) {
					sendNotification(IC_LEVEL_ORANGE, Constants.NOTIFICATION_STARTED);
				}
			}
			...
			public void registrationState(LinphoneCore lc, LinphoneProxyConfig cfg, LinphoneCore.RegistrationState state, String smessage) {
				if (!mDisableRegistrationStatus) {
					if (state == RegistrationState.RegistrationOk && LinphoneManager.getLc().getDefaultProxyConfig() != null && LinphoneManager.getLc().getDefaultProxyConfig().isRegistered()) {
						sendNotification(IC_LEVEL_ORANGE, Constants.NOTIFICATION_REGISTERED);
					}
...
//			inconId = R.drawable.topbar_call_notification;
			notificationText = Constants.INCALL_NOTIF_ACTIVE;
			break;
		case PAUSE:
//			inconId = R.drawable.topbar_call_notification;
			notificationText = Constants.INCALL_NOTIF_PAUSED;
			break;
		case VIDEO:
//			inconId = R.drawable.topbar_videocall_notification;
			notificationText = Constants.INCALL_NOTIF_VIDEO;
			break;	
		default:
		...
		mIncallNotif = Compatibility.createInCallNotification(getApplicationContext(), mNotificationTitle, notificationText, inconId, bm, name, mNotifContentIntent);
	...
	private Class<? extends Activity> incomingReceivedActivity = LinphoneLauncherActivity.class;
	...
	public synchronized void sendNotification(int level, String text) {
		...
		mNotif = Compatibility.createNotification(this, mNotificationTitle, text, R.drawable.icon2, 0, bm, mNotifContentIntent, true,notifcationsPriority);
	...
	public void onTaskRemoved(Intent rootIntent) {
		if (Constants.KILL_SERVICE_WITH_TASK_MANAGER) {
